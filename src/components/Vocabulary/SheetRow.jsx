...
const SheetRow = React.memo(({ item, columnOrder, columnDefs, columnVisibility, hiddenColumns, revealedCells, selectedIds, isPlaying, index, isEditMode, onUpdateCell, onRevealCell, onPlaySingle, onMark, isActive, allTags, searchTags, createTag, getTagName, toggleRowTag, isAuthenticated, searchTerm }) => {
    ...
    const renderCellContent = (colId) => {
        ...
        switch (colId) {
            case 'selection':
                return (
                    <td data-col-id={colId} className={`w-10 px-2 py-0.5 border-r border-slate-200 dark:border-white/5 ${isActive ? '!bg-[#e5f5f1] text-slate-900 dark:!bg-transparent dark:text-indigo-200' : 'bg-white/50 dark:bg-white/[0.02]'} text-center font-medium text-slate-500 text-xs`}>
                        {index + 1}
                    </td>
                );
            case 'audio':
